...
## Current state — 2026-09-11 (`8cfdefc` + Save my applications 2026-09-11 — 18 routes — deployed `rin-setu-git-main-dewang-chirlekars-projects.vercel.app` gated + intent-filter + Phase 0 check — no margin/subsidy/treatment in single-page PDF + 172p Lending Policy, stays placeholder)
...
| dev server | `npm run dev` | `/`, `/apply`, `/result`, `/personas`, `/applications`, `/admin/health-upload` + `/api/extract`/`/api/applications` (Gemini 2.5-flash / fixture, 503→fallback) render |
| prod build | `npm run build` | compiled, **18 routes** (`ƒ /api/extract`, `ƒ /api/extract-documents`, `ƒ /api/explain`, `ƒ /api/packet`, `ƒ /api/applications`, `ƒ /applications`, `ƒ /applications/[id]`, `ƒ /api/admin/*` ×5, `ƒ /admin/health-upload`), First Load JS 103 kB |
| db | `DATABASE_URL` (pooler 6543) + `npm run seed` | **connected** — `20260905144514_init` applied, **5 schemes** / 15 partners / 15 health rows / 53 doc reqs + **applications** · DB parity `src/lib/dataset-db.ts` verified (offline-graceful), `src/lib/db.ts:1` singleton `getPrisma()` |
...
| i18n | `next-intl` + `hi`/`mr` 297 keys | **accurate** `hi`/`mr`, globe `LocaleSwitcher`, brand `RinSetu` locked, tab stays English, `amy/uny` + `free_text_retry` + `partner.map.offline_*` + `applications` added |
...
| applications | `DATABASE_URL` + `/applications` | **done 2026-09-11** — `POST /api/applications` (search→`recommend()`→`Application` receipt `prisma/schema.prisma:367` `result` JSON + `figuresAuthoritative`/`datasetLabel`) + `GET /applications` list + `GET /applications/[id]` receipt view (never re-priced, `DatasetBanner` from stored `figuresAuthoritative`), `SaveApplicationButton.tsx:1` on `/result` |
| deploy | `Vercel` `rin-setu-git-main-dewang-chirlekars-projects.vercel.app` (gated) | **live 2026-09-11** — `next build` 18 routes, env `DATABASE_URL` pooler `6543` + `GEMINI_API_KEY` + `ADMIN_PASSWORD` + `DEMO_MODE=true`; DB already 5/15/15+53 via `npm run seed`; currently **Vercel Authentication gated** (SSO wall) — disable `Settings → Deployment Protection → Vercel Authentication` to go public |
...
Dataset: **5 schemes** (MICRO, TERM, EDU, **AMY Aajeevika 15%** + **UNY Udyam Nidhi 13%** — all `verified: false`, from `https://nsfdc.nic.in/scheme` 09.09.2026: MICRO 1.40L/1.25L 6.5% 3y, TERM 50L/45L 8% 7y, EDU 40L 6.5% 12y, AMY 1.40L/1.25L 15% 3y via NBFC-MFI, UNY 5L/4.5L 13% 5y via Coop; overlap **P41 tailoring 90k → MICRO+AMY both ELIGIBLE, ranking picks MICRO** `F2` live; **checked 2026-09-11** single-page schemes PDF `2026-06-15` + LENDING POLICY 172p `2026-05-15` — no explicit `margin`/`subsidy`/`moratorium_interest_treatment` found, stays `placeholder TODO(verify)` per hard rule 1), 15 fabricated partners on real city coordinates, 15 `SIMULATED` health rows (replaceable via `/admin/health-upload` CSV → `MIS_UPLOAD`), 297 i18n keys (`en`/`hi`/`mr`, + `amy/uny` + `free_text_retry` + `offline_*` + `applications` + intent-filter `result/page.tsx:180`), **41 persona fixtures** (P41 multi-eligible), `india-states-districts.json` (36 states/UTs, every district, MH 36×7–16 tehsils + 364 tehsils→villages, now till tehsil). Figures authoritative: **false** for two independent reasons (non-citable figures + demo overlay active). Applications: `Application` `prisma/schema.prisma:367` stores `applicant`+`result` JSON receipt (never re-priced) + `applications` list/detail routes `src/app/applications/page.tsx:1`.
...
**UI baseline — `src/app/` + `src/components/`** (Phase 3–6 baseline `4a8eea7`+`5eb1b1e`, Map 2026-09-05, PDF 2026-09-05, i18n 2026-09-05 — globe + brand lock + accurate `hi`/`mr` + form till tehsil + LLM 2.5-flash + purpose flat + **Save 2026-09-11**):
...
- `/applications` — **saved receipts** `src/app/applications/page.tsx:1` list (100 latest, ledger, `ui.applications.*` 22 keys, empty state with `needs_db_*`), `GET /api/applications` JSON; `/applications/[id]` `src/app/applications/[id]/page.tsx:1` shows stored `applicant`+`result` exactly as saved (never re-priced, `DatasetBanner` from stored `figuresAuthoritative`/`datasetLabel`), `GET /result` link via `applicantToParams`.
...
**Save my applications — `src/lib/db.ts:1` + `src/app/api/applications/route.ts:1`** (2026-09-11): `getPrisma()` singleton (`PrismaPg` pool, `ssl:{rejectUnauthorized:false}`, `hasDatabaseUrl()`/`isDbUnreachableError()` 503 graceful), `POST /api/applications` (`search` or `applicant` → `parseApplicantParams` → `recommend()` → `Application` `prisma/schema.prisma:367` stores `applicant`+`result` JSON receipt + `schemeCode`/`figuresAuthoritative`/`datasetLabel`/`language`, 201 `{id}`), `GET /api/applications` (100 latest). UI `src/app/applications/page.tsx:1` ledger list + `src/app/applications/[id]/page.tsx:1` receipt view (never re-priced, `DatasetBanner` from stored fields, `applicantToParams` → `/result`/`/api/packet` re-open), `SaveApplicationButton.tsx:1` on `/result` (disabled 503 when DB missing). No auth for demo (intentionally open). Works offline for list fallback `needs_db_*` i18n.
...
- **Phase 0 — finish transcription:** fetch `NSFDC Compendium 2024-05-15` `nsfdc.nic.in/UploadedFiles/.../1-4-1.pdf`, fill `margin_pct`, `subsidy_pct/cap/timing`, `moratorium_interest_treatment` for `MICRO/TERM/AMY/UNY` (now `placeholder` `TODO(verify)`), re-run `npm run verify:report` → aim for `MICRO/TERM` → `verified:true` and `figures_authoritative` flip for at least those 2. No code change, only `data/schemes.seed.json` values + `source_url/date`. Checked 2026-09-11 PDFs — no explicit `margin/subsidy/treatment` found, stays placeholder.
- **Form choice fix (done):** `LoanConfirmation` now lets user **pick any `ELIGIBLE`**, not just recommended — `P41` shows `MICRO 6.5%` vs `AMY 15%` radios `src/components/LoanConfirmation.tsx:9` `src/app/result/page.tsx:257`.
- **Save my applications (done 2026-09-11):** `POST /api/applications` on `/result` → `GET /applications` list + `/applications/[id]` receipt view `src/app/api/applications/route.ts:1` `src/lib/db.ts:1` `SaveApplicationButton.tsx:1` (DB already 5/15/15, no auth for demo, 503 graceful when `DATABASE_URL` missing). So manual form isn't lost if you lose the link `src/lib/applicant-params.ts:122` — **now wired, see `/applications`**.

**Next (3–5 days):**
- **Deploy unlock:** `Vercel` → `Settings → Deployment Protection → Vercel Authentication → Disable` to go public (currently gated), `DEMO_MODE` rehearsal `docs/ROADMAP.md:391` 4-min script, record demo video.
- **Polish:** Lighthouse a11y ≥90, XLSX alongside CSV, voice/Bhashini (optional), replace `PLACEHOLDER_petty_trade` with real `indicative-activities` flat list (you asked purpose should be generic, not scheme-picked — done `ApplyForm.tsx:59`), wire `LoanConfirmation` chosen code into save, `DELETE` for saved rows.
- **Phase 0 still blocked** on written `moratorium_interest_treatment` — ask nodal officer for circular that states `CAPITALISED/SERVICED/WAIVED` per scheme.

We are at **code-complete + Save 2026-09-11**: deterministic, UI (18 routes), DB (applications receipt), LLM `2.5-flash` fallback, i18n 297 keys, admin + save, offline paths work **and 5 NSFDC schemes transcribed**; remaining `TODO(verify)` need compendium. See `docs/PROGRESS.md` §7–§8 and `docs/ROADMAP.md` §10–§14 for step-by-step.
...
│  ├─ app/   layout.tsx  page.tsx  apply/page.tsx  result/page.tsx  personas/page.tsx  applications/page.tsx  applications/[id]/page.tsx  admin/health-upload/page.tsx  api/extract/route.ts  api/extract-documents/route.ts  api/explain/route.ts  api/packet/route.ts  api/applications/route.ts  api/admin/login|logout|health-preview|health-apply|health-template/route.ts
│  ├─ components/  ui.tsx  DatasetBanner.tsx  SchemeCard.tsx  VerdictList.tsx  LoanFigures.tsx  PartnerPanel.tsx  PartnerMap.tsx  ChecklistPanel.tsx  LocaleSwitcher.tsx  FreeTextIntake.tsx  LoanConfirmation.tsx  DocumentUpload.tsx  SaveApplicationButton.tsx  ExplainPanel.tsx  AdminHealthUpload.tsx  AdminLogin.tsx  form.tsx  ScrollReveal.tsx
│  ├─ lib/   dataset.ts  dataset-db.ts  db.ts (Prisma singleton, isDbUnreachableError)  packet.tsx  locale.ts  applicant-params.ts (lakh/thousand parse)  format.ts  view.ts  admin-auth.ts  health-csv.ts (dedupe fix)
...
│  └─ messages/  en.json (297 keys: +amy/uny + retry/confirm/offline + applications)  hi.json  mr.json  index.ts (strict throw, + next-intl, + globalThis.__RINSETU_LOCALE__)
