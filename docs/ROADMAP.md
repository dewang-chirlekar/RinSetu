...
> **Progress through 2026-09-05 (Maps done):** Phases **0–2 complete, 3–6 baseline shipped** (`4a8eea7` + `5eb1b1e` + `bd12a83`) + **DB connected + parity** (Supabase pooler 6543, `20260905144514_init` applied, `src/lib/dataset-db.ts` JSON↔DB deep-equal) + **core hardened** (A3 format pinned, `verified` derived) + **Maps** (`src/components/PartnerMap.tsx:1` MapLibre `6.7.0` + OSM raster, eligible-only). Deterministic core, **237 tests** (11 files, 1 skipped without DB) green, `npm run personas` and `/result` (now with map) work end-to-end. UI baseline renders `/`, `/apply` (GET→`/result`), `/result` (verdict table + finance + **map** + partners + checklist), `/personas`. Deferred: PDF packet, `next-intl` hi/mr, `src/llm/` extract+explain, admin health upload, offline demo hardening. See `docs/PROGRESS.md` §2/§5 for the gate.
