...
**Document owner:** _________ · **Last updated:** 2026-09-06 · **HEAD:** `bd12a83` on `main` + DB+LLM 3.6-flash + form till tehsil — **paused for tomorrow**

> **Progress through 2026-09-06 (paused for tomorrow — where we are):** Phases **0–2 complete, 3–6 baseline shipped** (`4a8eea7` + `5eb1b1e` + `bd12a83`) + **DB connected + parity** (Supabase pooler 6543, `20260905144514_init`, `dataset-db.ts` JSON↔DB deep-equal) + **core hardened** (A3 `age 18-60` pinned, `verified` derived) + **Maps** (`PartnerMap` MapLibre `6.7.0` OSM) + **PDF** (`packet` + `/api/packet`) + **i18n** (`next-intl` `hi`/`mr` 265 keys accurate, globe `LocaleSwitcher`, brand `RinSetu` lock, tab stays `en`, `layout`/`result`/`apply`/`personas`/`packet` locale-aware) + **form till tehsil** (`india-states-districts.json` 36 states + every district, MH 36×7–16 tehsils + 364 villages, `ApplyForm` `state→district→tehsil` cascade `tehsil→district→state` auto-detect, `village` removed per request, `FreeTextIntake` fills form + stays on `/apply` + missing nudge) + **LLM** (`src/llm/` `gemini-3.6-flash` (was 1.5 404, `ListModels` 200, quota 20/min `429` retry), `extract` `name`/`tehsil`/`village` + `normalizeExtracted`, `/api/extract`/`/api/extract-documents` + `DocumentUpload`/`LoanConfirmation`/`ExplainPanel`, `GEMINI_API_KEY` in `.env`). Deterministic core, **237 tests** (11 files, 1 skipped without DB) green, `npm run personas` and `/result` (map+PDF+explain+upload) work end-to-end. UI baseline renders `/`, `/apply` (free-text + guided, till tehsil), `/result` (verdict + finance + map + partners + checklist + PDF + explain + upload), `/personas`. **Next for tomorrow:** **Admin health upload (Phase 8)** *or* **Demo hardening (Phase 9)** — pick one. See `docs/PROGRESS.md` §2/§5/§7 for the gate.
