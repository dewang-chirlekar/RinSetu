...
**Document owner:** _________ · **Last updated:** 2026-09-05 · **HEAD:** `bd12a83` on `main` + DB connected

> **Progress through 2026-09-05 (i18n done):** Phases **0–2 complete, 3–6 baseline shipped** (`4a8eea7` + `5eb1b1e` + `bd12a83`) + **DB connected + parity** (Supabase pooler 6543, `20260905144514_init` applied, `src/lib/dataset-db.ts` JSON↔DB deep-equal) + **core hardened** (A3 format pinned, `verified` derived) + **Maps** (`PartnerMap.tsx:1` MapLibre `6.7.0` + OSM raster, eligible-only) + **PDF** (`packet.tsx:1` + `/api/packet`) + **i18n** (`next-intl` + `hi.json`/`mr.json` 265 keys, `LocaleSwitcher`, `layout`/`result`/`packet` locale-aware, glossary kept English). Deterministic core, **237 tests** (11 files, 1 skipped without DB) green, `npm run personas` and `/result` (now with map + PDF + hi/mr switcher) work end-to-end. UI baseline renders `/`, `/apply` (GET→`/result`), `/result` (verdict table + finance + **map** + partners + checklist + PDF), `/personas`. Deferred: `src/llm/` extract+explain, admin health upload, offline demo hardening. See `docs/PROGRESS.md` §2/§5 for the gate.
...
| **2** | **The heart.** Calculator + eligibility engine + remediation, headless, with golden and branch tests. No UI at all. | 4 | All 40 personas produce correct results from a CLI script · **✓ done + hardened 2026-09-05 — 237 tests (11 files), `npm run personas` prints 40 rows, A3 format pinned, `verified` derived, DB parity loader.** |
...
| **4** | Partner registry, geocoding, MapLibre map, hard filters, health score, dual ranking. | 3 | Zero capability leakage; simulated chips visible · **✓ done 2026-09-05 — hard filters + haversine + health scoring + dual ranking + `PartnerMap.tsx:1` (MapLibre `6.7.0` + OSM `tile.openstreetmap.org`, eligible-only markers, `fitBounds`, 240/320px, `npm run build` 8 routes).** |
...
| **6** | **Differentiator B** — document checklist + pre-filled PDF packet + partner notification. | 3 | A real PDF downloads with correct computed figures · **✓ done 2026-09-05 — checklist resolver + `ChecklistPanel` done + `src/lib/packet.tsx:1` (`PacketDocument`, ledger) + `src/app/api/packet/route.ts:1` (`GET /api/packet?...` → `application/pdf`, `renderToBuffer`) + `src/app/result/page.tsx:178` download link; reuses `recommend()` output, no new numbers, `npm run build` 9 routes.** |
| **7** | Multilingual: catalogs, LLM explanation in target language, do-not-translate glossary. | 3 | Full flow in every shipped language; no scheme name or amount ever translated · **✓ done 2026-09-05 — `next-intl` + `hi.json`/`mr.json` (265 keys, UI `hi`/`mr` human, scheme/amount glossary kept English) + `src/i18n.ts:1` + `LocaleSwitcher.tsx:1` + `layout`/`result`/`packet` locale-aware, `npm run build` all `ƒ` dynamic, `partner.map.` allowlisted.** |
